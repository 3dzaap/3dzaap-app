...
Mantenha o tom profissional, vibrante e encorajador.
  ...
  async generateCatalogInsights(catalogData) {
    const prompt = `
Você é um analista especialista em engenharia de produto e precificação de fazendas de impressão 3D.
Analise os indicadores do meu catálogo de produtos e forneça recomendações rápidas de melhoria.

DADOS DO CATÁLOGO:
- Total de Peças Cadastradas: ${catalogData.totalProducts}
- Preço Médio do Catálogo: ${catalogData.avgPrice} €
- Margem Média Estimada: ${catalogData.avgMargin}%
- Produtos Sem Giro (0 vendas registradas): ${catalogData.stagnantCount}
- Campeão de Vendas (Volume): ${catalogData.bestSeller || 'N/A'}
- Campeão de Eficiência (Margem/Lucratividade): ${catalogData.topEfficiency || 'N/A'}

A sua resposta deve ser em formato HTML simples. Use tags <strong>, <p>, <ul>, <li>. Mantenha sucinto, estratégico e elegante.
1. <p>Um parágrafo curto de diagnóstico sobre a margem e precificação geral do catálogo.</p>
2. <h4 style="margin: 10px 0 6px 0; font-size: 0.95rem; color: #a855f7;"><i class="ph-bold ph-lightbulb"></i> Plano de Ação Rápido:</h4>
3. Duas (2) sugestões práticas sobre o que fazer com os produtos sem giro e como maximizar as vendas do produto mais eficiente.
    `;
